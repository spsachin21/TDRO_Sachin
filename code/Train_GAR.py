...
def train_TDRO(dataloader, model, optimizer, n_group, n_period, loss_list, w_list, mu, eta, lamda, beta_p):
    ...
    for user_tensor, item_tensor, group_tensor, period_tensor in dataloader:
        ...
        # For GARRec, we reference the generator's second Linear layer.
        grad_ge = torch.zeros((n_group, n_period, model.generator[2].weight.reshape(-1).size(0))).cuda()
        
        # Iterate over model parameters to find the target parameter.
        for name, param in model.named_parameters():
            # Check for the weight of the final linear layer in the generator.
            if name == 'generator.2.weight':
        ...
        # Compute worst-case factor: average group loss normalized by group sample counts.
        de = torch.tensor([torch.sum(group_tensor == g_idx) for g_idx in range(n_group)]).cuda()
        loss_ = torch.sum(loss_ge, dim=1) / (de + 1e-16)
        
        # Compute shifting factor (trend) from group gradients.
        trend_ = torch.zeros(n_group).cuda()
        for g_idx in range(n_group):
            g_j = torch.mean(grad_ge[g_idx], dim=0)  # Mean gradient for group g_idx.
