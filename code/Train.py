...
import torch.nn.functional as F  #This line imports the torch.nn.functional module, which contains a variety of functions commonly used in neural networks, such as activation functions (F.relu, F.sigmoid), loss functions (F.mse_loss, F.cross_entropy), and convolutional/pooling operations.
from torch.autograd import grad # for automatic differentitaion in training

def train_ERM(dataloader, model, optimizer):    
    model.train()
    for user_tensor, item_tensor in dataloader:
        optimizer.zero_grad()
        loss = model.loss(user_tensor.cuda(), item_tensor.cuda())  #computes a ranking loss (e.g., BPR, hinge, contrastive) or binary cross-entropy between predicted and true interactions.
        loss.backward()  #Computes gradients of the loss with respect to all model parameters using autograd- This enables the model to learn how to update weights.
        optimizer.step()  #update the model weights
    return loss

def train_TDRO(dataloader, model, optimizer, n_group, n_period, loss_list, w_list, mu, eta, lamda, beta_p):

    model.train() #Sets the model to training mode, enabling features like gradient tracking
    ...
    for user_tensor, item_tensor, group_tensor, period_tensor in dataloader:
        optimizer.zero_grad()
    #model's prediction error (sample_loss) and any regularization penalty (reg_loss)
        ...
        for name, param in model.named_parameters():
            if name == 'encoder_layer2.weight':  # ensures that the following code block only executes for the weights of the layer named 'encoder_layer2'.
        ...
        update_factor = eta * loss_list # weight steo size
        w_list = w_list * torch.exp(update_factor)   # it is wi value 
        ...
        loss_weightsum.backward()  #It calculates the gradients of the loss_weightsum with respect to all the model's trainable parameters
        optimizer.step()    #updates the model's parameters based on the calculated gradients.

        loss_list.detach_()    #means it won't be considered for gradient calculations in future iterations. It's done to prevent unnecessary memory usage and potential errors.
        w_list.detach_()  # same

    with torch.no_grad():   #ensures that no gradient calculations are performed within it
